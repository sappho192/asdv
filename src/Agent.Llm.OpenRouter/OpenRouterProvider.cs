...
namespace Agent.Llm.OpenRouter;
...
public class OpenRouterProvider : IModelProvider
{
    ...
    public void SetModelCapabilities(string modelName)
    {
        try
        {
            var response = _httpClient.GetAsync($"{_baseUrl.TrimEnd('/')}/models")
            ...
            {
                if (!model.TryGetProperty("id", out var idEl)) continue;
                if (!string.Equals(idEl.GetString(), modelName, StringComparison.OrdinalIgnoreCase)) continue;

                if (model.TryGetProperty("context_length", out var ctxEl))
                {
                    var ctx = ctxEl.GetInt32();
                    Capabilities = new OpenRouterCapabilities(ctx, null);
                }
                break;
            }
        }
        ...
        {
            // Leave Capabilities as null
        }
    }

    private sealed record OpenRouterCapabilities(int? MaxContextTokens, int? MaxOutputTokens) : IProviderCapabilities;
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        string? openRouterCost = null;
        if (response.Headers.TryGetValues("x-openrouter-cost", out var costValues))
        {
            openRouterCost = costValues.FirstOrDefault();
        }
        ...
        // Emit cost trace if available
        if (!string.IsNullOrEmpty(openRouterCost))
        {
            yield return new TraceEvent("openrouter_cost", openRouterCost);
        }
    ...
    private HttpRequestMessage BuildRequest(ModelRequest request)
    {
        ...
        var httpRequest = new HttpRequestMessage(HttpMethod.Post, $"{_baseUrl.TrimEnd('/')}/chat/completions")
        {
        ...
        httpRequest.Headers.Add("HTTP-Referer", "https://github.com/asdv");
        httpRequest.Headers.Add("X-Title", "asdv");
